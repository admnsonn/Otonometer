...
const Utakmain = () => {
  // useEffect(()=>{
  //   ReactGA.send({ hitType: "pageview", page: window.location.pathname, title: "Utak-Atik" });
  // },[])
  ...
  function clickElement(target, retryCount = 0) {
    ...
    if (element) {
      ...
      console.error(
        `Element with id ${target} not found after ${maxRetries} attempts.`
      );
    }
  }
  ...
  function getDataSet2() {
    ...
    sessionStorage.setItem(
      ...
      JSON.stringify({
        ...
        dataset_1:
          dataSet1.length === 0 ? live_data?.dataset_1 || [] : dataSet1,
        all_ds1_set:
          ds1_set.length === 0 ? live_data?.all_ds1_set || [] : ds1_set,
        ...
        all_ds2_set: all_ds2,
        labelset1:
          labelSet1.length === 0 ? live_data?.labelset1 || [] : labelSet1,
        labelset2: labels,
          ...
          menu[0].forEach((item) => {
            ...
            if (checkbox) {
              ...
              while (!status && index < menu[2].length) {
                let relatedCheckbox = document.getElementById(
  ...
  useEffect(() => {
    const liveData = JSON.parse(sessionStorage.getItem("selectedData")) || {};
    setData(liveData);
  }, []);

  const handleClick = () => {
    const live_data = JSON.parse(sessionStorage.getItem("selectedData")) || {};
    ...
    sessionStorage.setItem("selectedData", JSON.stringify(updatedData));
    setData(updatedData);
      ...
      <div className="flex flex-col mt-[100px] mb-[150px] justify-center items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-[40px] gap-y-[10px] mt-[20px]">
          ...
          <div className="w-[250px] h-auto text-secondary font-medium text-[14px] cursor-pointer">
            <div
              onClick={() => {
                if (selectedCity.toLowerCase() !== "semua" && isDataset2Clicked)
                  return;
              ...
              className={`bg-[#ebebeb] w-full p-2 px-[30px] flex items-center justify-between rounded-[10px] ${
                selectedCity.toLowerCase() !== "semua" && isDataset2Clicked
                  ? "cursor-not-allowed opacity-50"
                  : ""
              }`}
              // className="bg-[#ebebeb] w-full p-2 px-[30px] flex items-center justify-between rounded-[10px]"
              // disabled={selectedCity.toLowerCase() === "semua" && isDataset2Clicked}
            >
              {selected
                ? selected?.length > 20
          ...
          <div className="flex gap-[10px]">
            <div className="hover-container">
              <img
                src={dataranicon}
                alt=""
                className="object-contain w-[60px] lg:w-20"
              />
        ...
        <div id="dataset1" className="gap-[100px] mt-[40px] h-auto">
          <div>
            <h1 className="text-secondary text-[14px] font-semibold lg:ml-[100px] ml-[50px]">
              DATASET 1
            </h1>
            <div className="flex gap-x-[20px]">
              <div className="lg:w-[250px] w-[200px] h-auto rounded-[10px] text-white border-1 border-[f1f1f1] text-[14px] font-medium items-center justify-center drop-shadow-lg mt-[10px] mb-[10px] cursor-pointer lg:ml-[20px] ml-[-10px] md:ml-[-10px]">
                <div
                  onClick={() => {
                    handleParentClick();
                  ...
                  >
                    ...
                    {selectFilter?.map((filternya) => (
                      <li
                        ...
                        className={`p-2 text-[12px] hover:bg-[#a4b6b9] hover:text-secondary rounded-[10px] text-center`}
                        onClick={() => {
                          // if (filternya.contain_alert == "y") {
                          //   popUpCheckbox(filternya.content_alert)
                          // }
                          updateSelectFilter(
                            filternya.nama,
                            selectedSelectFilter,
                ...
                <button
                  onClick={() => {
                    ...
                    const selectedElements1 = dataset1.querySelectorAll(
                      'input[type="checkbox"]:checked'
                    );
                    if (selectedElements1.length === 0) {
                      Swal.fire({
                        title: "Perhatian!",
                        text: "Anda harus memilih setidaknya satu pilihan dari dropdown.",
                        confirmButtonText: "Tutup",
                        confirmButtonColor: "#24445A",
                        customClass: {
                          icon: "no-border",
          ...
          <div>
            <div className="flex flex-row lg:flex-row md:flex-row lg:items-center lg:justify-center lg:gap-x-5 md:gap-x-2 mt-5 lg:mt-[0px]">
              <button
                ...
                className="lg:ml-[-60px] md:ml-[2px] ml-[-2px] mt-[-10px] lg:mt-[0px] md:mt-[-10px] lg:mb-[5px] relative"
              >
                <img
                  src={back}
                  alt=""
              ...
              <h1 className="text-secondary text-[14px] font-semibold lg:ml-[20px] ml-[30px] md:ml-[15px] mt-[-6px]">
                DATASET 2
              </h1>
            </div>
            <div className="flex gap-x-[20px]">
              <div className="lg:w-[250px] w-[200px] h-auto rounded-[10px] text-white border-1 border-[f1f1f1] text-[14px] font-medium items-center justify-center drop-shadow-lg mt-[10px] mb-[10px] cursor-pointer ml-[-10px] md:ml-[-10px] lg:ml-[20px]">
                <div
                  onClick={() => {
                    handleParentClick2();
                ...
                <button
                  onClick={() => {
                    ...
                    const selectedElements2 = dataset2.querySelectorAll(
                      'input[type="checkbox"]:checked'
                    );
                    ...
                    if (
                      document.getElementById(`ds2_all_${IDchildFilter2}`) !==
                      null
                    ) {
                      if (
                        !document.getElementById(`ds2_all_${IDchildFilter2}`)
