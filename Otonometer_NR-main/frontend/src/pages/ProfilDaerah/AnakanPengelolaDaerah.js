...
import { faArrowDown, faArrowDownShortWide, faChevronDown, faChevronLeft, faChevronRight, faClose, faPhone } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { NavLink, useParams } from 'react-router-dom';
import { getLayananPublik, getListJabatanALL, getListJabatanSpesific } from './services';
import CardPejabatSupervisor from './cardPejabatSupervisor';
import './layananPublikStyle.css';

const AnakanPengelolaDaerah = () => {
  const {id} = useParams();
  
  const [dataCardPimpinan, setDataCardPimpinan] = useState([])
  const [dataCardPimpinanTertinggi, setDataCardPimpinanTertinggi] = useState([])
  ...
  useEffect(()=>{
    getListJabatanALL((data)=>{
      setDataCardPimpinanTertinggi(data.data)
    })
  },[])

  useEffect(()=>{
    getListJabatanSpesific(id,(data)=>{
      const superVisornya = data.data[0].atasan.filter(item => item);
      console.log(superVisornya)
      setDataCardPimpinan(superVisornya)
      setOnSelected(data.data)
        ...
        <div className="flex flex-col mt-[125px] justify-start items-center min-h-screen h-auto">
              ...
              <div className='flex flex-col w-full max-w-[990px] items-start justify-start'>
                <div className='flex w-full'>
                    {CardPejabatSupervisor(dataCardPimpinan, dataCardPimpinanTertinggi)}
                </div>
...
export default AnakanPengelolaDaerah